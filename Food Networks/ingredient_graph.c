...
intNode *mutation(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist, intNode *avoid)
{
    if ( dist != k) {
      int i = ingredient_index(source_ingredient);
      if (i != -1 ) {
        if (dist == 0 && k - dist > 1 && searchInt(avoid, i) == 0) {
          h = insertInt(h, i);
        }
        int j = 0;
        while (j<MAT_SIZE) {
          if (AdjMat[i][j] != 0) {
            h = mutation(h, ingredients[j], k, dist+1, avoid);
            if (searchInt(h, j) == 0 && searchInt(avoid, j) == 0) {
              h = insertInt(h, j);
            }
          }
          j++;
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
     ...
     * Implement this function
     *****/
    intNode *to_avoid = NULL;
    to_avoid = related_k_dist(to_avoid, avoid, k_avoid, 0);
    to_avoid = insertInt(to_avoid, ingredient_index(avoid));
    intNode *h = NULL;
    h = mutation(h, source_ingredient, k_source, 0, to_avoid);
    deleteList(to_avoid);
    return h;    
}
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
  ...
  int x = 0;
  while (x!= 10 && strcmp(recipe[x], "") != 0 ) {
    if (strcmp(recipe[x], to_change) != 0) {
      counter[ingredient_index(recipe[x])] = -1;
    }
    x++;
  }
  counter[ingredient_index(recipe[x-1])] = -2;
  int highest = 0;
  int ind = 0;
  int maybe;
  ...
  {
    if (strcmp(recipe[ix], to_change) == 0) {
      change_index = ix;
    }
  }
  if (change_index != -1) {
   strcpy(recipe[change_index], ingredients[ind]); 
  }
  
}

void print_mat(double mat[MAT_SIZE][MAT_SIZE]) {
  for (int i = 0; i < MAT_SIZE; i++)
  {
    for (int j = 0; j < MAT_SIZE; j++)
    {
