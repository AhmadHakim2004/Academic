...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
     ...
     ****/
	double key_yarab = (10.0*bar)+index;
	if (root==NULL) {return NULL;}
	if ( root -> key == ( key_yarab ) ) {return root;}
	if  ( root -> key > key_yarab) {
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
     ...
     * Implement this function
     ****/
	double key_to_search = (10.0*bar)+index;
	BST_Node *tmp;
	if (root == NULL) {
		return NULL;
	}
...
void delete_BST(BST_Node *root)
{
	...
	BST_delete(root, root->bar, root->index);
	}
}

void switch_values(BST_Node *root, int min, int max) {
	if (root != NULL) {
		switch_values(root->left, min, max);
		root->bar = min+max - root->bar;
		root -> index = 1 - root -> index;
		double new_key = (10*( root->bar) )+ root->index;
		root->key = new_key;
		switch_values(root->right, min, max);
	}
}

void reverse_tree(BST_Node *root) {
	if (root != NULL) {
		reverse_tree(root->left);
		reverse_tree(root->right);
		BST_Node *leftie = NULL;
		BST_Node *rightie = NULL;
		leftie = root->left;
		rightie = root->right;	
		root -> left = rightie;
		root-> right = leftie;
...
BST_Node *reverseSong(BST_Node *root)
{
	...
	int MIN_BAR = -1;
	int MAX_BAR = -1;
	BST_Node *to_left = NULL;
	BST_Node *to_right = NULL;
	to_left = root;
	to_right = root;
	while (to_left != NULL) {
		MIN_BAR = to_left ->bar;
	...
	while (to_right != NULL) {
		MAX_BAR = to_right ->bar;
		to_right = to_right->right;
	}
	switch_values(root, MIN_BAR, MAX_BAR);
	reverse_tree(root);
	return root;
}
...
LL_Node *newLL_Node(double freq, int bar, double index)
{
        
	LL_Node *new_review=NULL;
	new_review=(LL_Node *)calloc( 1, sizeof(LL_Node) );
	new_review->freq = freq;
	new_review->bar = bar;
	new_review->index = index;
	new_review->key = (10.0*bar)+index;
...
LL_Node *insert_to_LL(LL_Node *head, double freq, int bar, double index){
 	LL_Node *note_to_add = newLL_Node(freq, bar, index);
	note_to_add->next=head;
	return note_to_add; 
}
LL_Node *insert_LL_Node(BST_Node *root, LL_Node *head) {
	if (root != NULL) {
		head =insert_LL_Node(root->left, head);
		head=insert_to_LL(head,root->freq,root->bar,root->index);
		head =insert_LL_Node(root->right, head);
	}
	return head;
}
int j_finder(double freq, double f_array[100]) {
	int i = 0;
	while(i < 100) {
		if (f_array[i] - freq < 0.001 && f_array[i] - freq > -0.001) {
			return i;
		}
		i++;
	}
...
BST_Node *freq_index_checker(BST_Node *note, int semitones, double time_shift, double f_array[100]) {
	int j = j_finder(note->freq, f_array);
	if (0<= j + semitones &&  j + semitones <= 99 && 0 <= note->index + time_shift && note->index + time_shift< 1) {
		BST_Node *new_note = newBST_Node(f_array[j+semitones], note->bar, note->index+time_shift);
		return new_note;
	}
	return NULL;
}
void deleteLL(LL_Node *head) {
	while (head != NULL) {
			LL_Node *p=NULL;
			p = head;
			head = head -> next;
	...
	
{
     ...
     ****/
	LL_Node *p=NULL;
	p = insert_LL_Node(root, p);
	LL_Node *p_2=NULL;
	p_2 = p;
	while (p_2 != NULL) {
		BST_Node *node_to_add = newBST_Node(p_2->freq, p_2->bar, p_2->index);
		node_to_add= freq_index_checker(node_to_add, semitones, time_shift, note_freq);

		if (node_to_add!= NULL) {
			root = BST_insert(root, node_to_add);
		}
		else {
		}
		p_2=p_2->next;
